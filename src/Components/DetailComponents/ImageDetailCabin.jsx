...
const ImageDetailCabin = () => {
    ...
    if (!getCabinDetail || !getCabinDetail.images || getCabinDetail.images.length === 0) {
        return <div>Bilder finns inte</div>;
    }

    const handleNextImage = () => {
        setCurrentImageIndex((prevIndex) => (prevIndex + 1) % getCabinDetail.images.length);
    };

    const handlePreviousImage = () => {
        setCurrentImageIndex((prevIndex) => (prevIndex - 1 + getCabinDetail.images.length) % getCabinDetail.images.length);
    };

    return (
        <div>
            <img className='detail-img' src={getCabinDetail.imageURL} alt="" />
